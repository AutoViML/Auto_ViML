...
def tokenize_test_by_metric(model, X_train, X_cv, y_train, y_cv,
                            metric, vect=None, modeltype='Classification', verbose=0):
    if vect is None:
...
def select_top_features_from_SVD(X, tsvd, is_train=True):
    """
...
# define a function that accepts a vectorizer and calculates its accuracy
################################################################################
def fit_and_predict(model, X_train, y_train, X_cv, is_train=True):
...
###########################################################################
def transform_combine_top_feats_with_SVD(nlp_column, big_nlp_vect, new_vect,
    ...
    if is_train:
        ...
        print(
            'Time Taken for Transforming Train %s data = %0.0f seconds' % (each_df_dtm.shape, time.time() - start_time))
    else:
...
def select_best_nlp_vectorizer(model, data, col, target, metric,
    ...
    if modeltype is None or modeltype == '':
        ...
        tvec = TfidfVectorizer(ngram_range=(1, 3), stop_words=None, max_features=max_features, min_df=min_df,
                               max_df=max_df)
        data_dtm = data[col]
    ...
    if data.shape[0] < 10000:
        count_max_df = 0
        print(f'Trying multiple max_df values in range {np.linspace(0.95, 0.05, 5)} to find best max_df...')
            ...
            if count_max_df == 0:
                ...
                if modeltype == 'Regression' or metric in ['logloss', 'log_loss']:
                    ...
                    if current_metric >= best_metric:
                        ...
                        break
            count_max_df += 1
        print(f'Best max_df selected to be {best_max_df}')
    else:
        best_max_df = 0.5
        print(
    ...
    ##########################################################################
    print(
        '\n#### Using Count Vectorizer with Latin-1 encoding, limited max_features =%d and a min_df=%s with n_gram (1-5)' % (
            max_features, min_df))
...
def Auto_NLP(nlp_column, train, test, target, score_type='',
    ...
    if train.shape[0] >= 100000:
        print(
            'Cleaning text in %s column. Please be patient since this is a large dataset with >100K rows...' % nlp_column)
    else:
    ...
    if tweets_flag:
        ...
        print(
            '    Faster text processing using clean_tweets function, since top_num_features exceeds %s' % top_num_features_limit)
        ...
        print(
            '    Faster text processing using clean_text function, since top_num_features is below %s' % top_num_features_limit)
        train[nlp_column] = clean_text(train[nlp_column])
    ###############################################################################################################
    ...
    if build_model:
        ...
        ########################################################################################
        if 'auto_nlp_source' not in train_nlp.columns.tolist():
            train_nlp['auto_nlp_source'] = 'Train'
        if type(test) != str:
            test_nlp[target] = 0
            if 'auto_nlp_source' not in test_nlp.columns.tolist():
        ...
        if len(nlp_data) <= 10000:
            ...
            nlp_data[senti_cols[2]] = nlp_data[nlp_column].apply(calculate_line_sentiment, 'subjectivity').fillna(
                0).values
...
def NLP_select_best_model_fit_predict(X, y, test, modeltype, score_type):
    ...
    if str(nlp_model).split("(")[0] == 'MultinomialNB':
        ...
        X_train = abs(X_train)
        X_test = abs(X_test)
    y_pred = fit_and_predict(gs, X_train, y_train, X_test, is_train=True)
    ...
    if str(nlp_model).split("(")[0] == 'MultinomialNB':
        ...
        if not isinstance(test, str):
            test = abs(test)
    if not isinstance(test, str):
        y_pred = fit_and_predict(gs, X, y, test, is_train=False)
...
def add_sentiment(data, nlp_column):
    ...
    cols = [nlp_column + '_vader_neg', nlp_column + '_vader_pos', nlp_column + '_vader_neu',
            nlp_column + '_vader_compound']
    print('    Created %d new columns using SentinmentIntensityAnalyzer. Time taken = %d seconds' % (
        len(cols), time.time() - start_time))
    return data, cols
